...
public class DLFSAdapterTest {
	...
	@Test
	public void widenedDispatchMatchesOperationForm() throws Exception {
		// #253: CoviaAPI's job-free REST routes call dispatch() directly
		// (bypassing invokeFuture's op dispatch), so it must enforce identical
		// authority/behavior as the v/ops/dlfs/* operation form.
		...
		ACell viaDispatch = dlfs.dispatch(ctx, "list", Maps.of("drive", "widened-check"));
		...
		AVector<?> drivesViaDispatch = RT.ensureVector(
			RT.getIn(dlfs.dispatch(ctx, "listDrives", Maps.empty()), "drives"));

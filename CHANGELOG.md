...
- `skills:list` degrades on an unreadable source (visible diagnostic) while `skills:read` still fails; `agent:create` warns when a declared skill source is missing or denied
- `agent:create` notes an empty own-workspace skill space (`w/skills empty …`) — a prompt for the agent to author its own skills there

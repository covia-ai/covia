...
- `model` facet on LLM operation assets carrying optional provider-specific `options` — rendering hints (`systemMessages`, `requiresUserMessage`, `cachePrefix`, `toolCallingByModel`) declared as data rather than branched on by provider name; reported by `v/ops/langchain/models`

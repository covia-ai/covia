...
- `v/info/url` (venue base URL) and `v/info/webdav` (`{enabled, url?, path?}`), plus `dlfs-webdav` in `v/info/protocols` when mounted — agents can now tell humans where the venue and its WebDAV live

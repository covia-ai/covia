...
4. **Create the bot.** Preferred — one op, no restart, persisted in your workspace:
   ...
   - **Pick the inbound handler** (ask the user which they want; exactly one). Every inbound message runs as a Job in the bot user's job index — that is the record of the interaction; the module keeps no log of its own:
     - `"agent": "<agentId>"` — each Telegram chat is one `agent:chat` conversation with that agent (create it first with `/agent create`). Always replies. The agent receives `{text, via: {from, chat, bot, access}}` — Telegram's authenticated sender as structure — so it knows who it is talking to; a system-prompt line like *"Telegram user @mikera (id 648055019) is Mike, the venue owner"* tells it what that identity *means* (the module states facts, the prompt states trust).
